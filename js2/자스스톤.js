...
function 카드돔연결(데이터, 돔, 영웅) {
  ...
  카드.addEventListener("click", function (card) {
    if (턴) {
      ...
      if (데이터) var idx = 내덱data.indexOf(데이터);
      console.log("내덱data" + 내덱data);
      내덱data.splice(idx, 1);
      내필드data.push(데이터);
      내덱.innerHTML = "";
      내필드.innerHTML = "";
...
//카드공장에 넣어버리면 카드공장을 실행할때마다 Card가 실행이됀다.
...
턴버튼.addEventListener("click", function () {
  턴 = !턴; //턴 -> false 면 true로 만들어서 대입하고 true이면 false로 만들어서 대입해라
